...
/**
 * Starts a webserver which serves packaged files for:
...
app.get("/", function (req, res) {
    res.sendfile(path.normalize(__dirname + "/../target/www/index.html"));
});

app.all(/^(.*)$/, function (req, res, next) {
    res.sendfile(path.normalize(__dirname + "/../target/www/" + req.params[0]));
});
...
function serverStarted () {
    console.log("Prod Server started on http://localhost:" + server.address().port);
}
